...
class HealthManager {
    ...
    func fetchWorkoutsForMonth(month: Date, completion: @escaping(Result<[Workout], Error>) -> Void) {
        ...
        let predicate = HKQuery.predicateForSamples(withStart: .startOfWeek, end: endDate)
        ...
        let query = HKSampleQuery(sampleType: workouts, predicate: predicate, limit: HKObjectQueryNoLimit, sortDescriptors: [sortDescriptor]) { _, results, error in
            ...
            let workoutArray = workouts.map( {Workout(id: nil, title: $0.workoutActivityType.name, image: $0.workoutActivityType.image, duration: "\(Int($0.duration)/60) mins", colorTint: $0.workoutActivityType.color, date: $0.startDate.formatWorkoutDate(), calories: ($0.totalEnergyBurned?.doubleValue(for: .kilocalorie()).formattedNumberString() ?? "-") + "kcal")} )
